...
class ModifiedResNet(nn.Module):
    def __init__(self, num_classes):
        ...
        # 替换 ResNet 的全连接层，但保留其他层
        self.resnet.fc = nn.Sequential(
            nn.Dropout(0.5),  # 在全连接层之前添加 Dropout
            nn.Linear(self.resnet.fc.in_features, num_classes)  # 新的全连接层
        )
    def forward(self, x):
...
class EarlyStopping:
    def __init__(self, patience=7, verbose=False, delta=0):
        ...
        loss_path = 'resnet_best_loss.json'
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'resnet_best_checkpoint.pt')
        # 保存loss记录
        loss_path = 'resnet_best_loss.json'
        print("[INFO]更新loss记录")
...
def main():
    ...
    # 检查是否有已保存的模型
    model_path = 'resnet_best_checkpoint.pt'
